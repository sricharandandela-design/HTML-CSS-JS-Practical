-- Implement EXISTS and NOT EXISTS.
...
SELECT s.* FROM students s WHERE EXISTS (SELECT 1 FROM enrollments e WHERE e.student_id=s.id);
SELECT s.* FROM students s WHERE NOT EXISTS (SELECT 1 FROM enrollments e WHERE e.student_id=s.id);
